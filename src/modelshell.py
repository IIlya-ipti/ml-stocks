...
def save_dict(dictionary, name):
    with open("resource/dicts/" + name + 'dict' + '.pkl', 'wb') as f:
        pickle.dump(dictionary, f)


def load_dict(name):
    with open("resource/dicts/" + name + 'dict' + '.pkl', 'rb') as f:
        loaded_dict = pickle.load(f)
        return loaded_dict


def save_token(tokenizer, name):
    with open("resource/tokens/" + name + '.pickle', 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)


def load_tokens(name):
    with open("resource/tokens/" + name + '.pickle', 'rb') as handle:
...
class ModelClassifier:
    def __init__(self, model=None, name=None, load=False):
        ...
        self.name = name
        if load:
            self.model = keras.models.load_model("resource/models/" + name)
