...
# print("WDFSDLPLF")
# asyncio.run(add_channel("+79218099960"))
# print("SDFSDF")


async def get_all_messages(url):
    global channels
    if len(channels) == 0:
...
def get_all_messages_with_(url):
    with client:
        return client.loop.run_until_complete(get_all_messages(url))


async def get_all_messages_with(url):
    async with client:
        return client.loop.run_until_complete(get_all_messages(url))


def save_train_data():
    arr = get_all_messages_with_("+79181084445")
